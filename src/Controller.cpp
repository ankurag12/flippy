...
bool Controller::balance(double distance_setpoint) {
  ...
  const double tilt_angle_setpoint_max_change = 5.0;
  ...
  double pwm_input = 0.5;
  const double left_motor_stiction_offset = 0.05;
  const double right_motor_stiction_offset = 0.055;
  double pwm_input_linear_speed = 0.0;
  const double pwm_input_linear_speed_max = 0.1;
  ...
  const int inner_loop_ctr_lim = 3;
  ...
  while (fabs(tilt_angle_filtered) < tilt_angle_max) {
    ...
    pwm_input += pwm_input_linear_speed;
    ...
    pwm_input_linear_speed = gain_distance.p * err_distance +
                             gain_distance.i * int_err_distance +
                             gain_distance.d * diff_err_distance;

    // Clip pwm_input_linear_speed
    if (pwm_input_linear_speed > pwm_input_linear_speed_max) {
      pwm_input_linear_speed = pwm_input_linear_speed_max;
    } else if (pwm_input_linear_speed < -pwm_input_linear_speed_max) {
      pwm_input_linear_speed = -pwm_input_linear_speed_max;
    }
    std::cout << "err_distance = " << err_distance
              << " pwm_input_linear_speed = " << pwm_input_linear_speed
              << std::endl;
  }
  stop();
